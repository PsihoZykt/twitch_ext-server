...
const STRINGS = {
    ...
    counterBroadcast: 'Broadcasting counter %s for c:%s',
...
ext.version(require('../package.json').version).option('-s, --secret <secret>', 'Extension secret').option('-c, --client-id <client_id>', 'Extension client ID').option('-o, --owner-id <owner_id>', 'Extension owner ID').parse(process.argv);
...
app.post('/counter', setCounterHandler)
app.get('/counter', getCounterHandler)
...
function getCounterHandler(req, res) {

    const payload = verifyAndDecode(req.headers.authorization);
    const {channel_id: channelId, opaque_user_id: opaqueUserId} = payload;
    const currentCounter = channelCounter[channelId] || initialCounter
    res.status(200).json({counter: currentCounter})
}
let attemptCounterBroadcast = (channelId) => {
    // Check the cool-down to determine if it's okay to send now.
    // const now = Date.now();
    // const cooldown = channelCooldowns[channelId];
    // if (!cooldown || cooldown.time < now) {
    // It is.
    sendCounterBroadcast(channelId);
    // channelCooldowns[channelId] = {time: now + channelCooldownMs};
    // } else if (!cooldown.trigger) {
    // It isn't; schedule a delayed broadcast if we haven't already done so.
    // cooldown.trigger = setTimeout(sendCounterBroadcast, now - cooldown.time, channelId);
    // }
}
function setCounterHandler (req,res){
    console.log("setCounterHandler")
    const payload = verifyAndDecode(req.headers.authorization);
    const {channel_id: channelId, opaque_user_id: opaqueUserId} = payload;

    // Store the color for the channel.
    let currentCounter = channelCounter[channelId] || initialCounter;
    ...
    // Broadcast the color change to all other extension instances on this channel.
    attemptCounterBroadcast(channelId)
    res.status(200).json({counter: currentCounter})
    // }
...
function sendCounterBroadcast(channelId) {
    ...
    verboseLog(STRINGS.counterBroadcast, currentCounter, channelId);
    request(
